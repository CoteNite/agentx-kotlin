...
interface AgentVersionRepository : JpaRepository<AgentVersionEntity, String> {
    ...
    // 根据代理ID查找版本
    fun findByAgentIdAndDeletedAtIsNullOrderByVersionNumberDesc(agentId: String): List<AgentVersionEntity>
    
    // 根据发布状态查找版本
    fun findByPublishStatusAndDeletedAtIsNull(publishStatus: Int): List<AgentVersionEntity>
    
    // 根据代理ID和发布状态查找版本
    fun findByAgentIdAndPublishStatusAndDeletedAtIsNull(agentId: String, publishStatus: Int): List<AgentVersionEntity>
    
    // 根据版本号查找
    fun findByAgentIdAndVersionNumberAndDeletedAtIsNull(agentId: String, versionNumber: String): AgentVersionEntity?
    
    // 根据创建者查找版本
    fun findByUserIdAndDeletedAtIsNull(userId: String): List<AgentVersionEntity>
    ...
    // 根据名称模糊搜索
    fun findByNameContainingIgnoreCaseAndDeletedAtIsNull(name: String): List<AgentVersionEntity>
    
    // 根据描述模糊搜索
    fun findByDescriptionContainingIgnoreCaseAndDeletedAtIsNull(description: String): List<AgentVersionEntity>
    
    // 根据创建时间范围查找
    fun findByCreatedAtBetweenAndDeletedAtIsNull(startTime: LocalDateTime, endTime: LocalDateTime): List<AgentVersionEntity>
    
    // 软删除
    @Query("UPDATE AgentVersionEntity av SET av.deletedAt = :deletedAt WHERE av.id = :id")
    fun softDeleteById(@Param("id") id: String, @Param("deletedAt") deletedAt: LocalDateTime): Int
    
    // 根据代理ID软删除所有版本
    @Query("UPDATE AgentVersionEntity av SET av.deletedAt = :deletedAt WHERE av.agentId = :agentId")
    fun softDeleteByAgentId(@Param("agentId") agentId: String, @Param("deletedAt") deletedAt: LocalDateTime): Int
    ...
    // 统计代理的版本数量
    fun countByAgentIdAndDeletedAtIsNull(agentId: String): Long
    
    // 统计指定发布状态的版本数量
    fun countByPublishStatusAndDeletedAtIsNull(publishStatus: Int): Long
    
    // 检查版本是否存在
    fun existsByIdAndDeletedAtIsNull(id: String): Boolean
    
    // 检查代理和版本号是否存在
    fun existsByAgentIdAndVersionNumberAndDeletedAtIsNull(agentId: String, versionNumber: String): Boolean
    
    // 查找代理的最新版本
    fun findTop1ByAgentIdAndDeletedAtIsNullOrderByVersionNumberDesc(agentId: String): AgentVersionEntity?
    
    // 查找代理的最新发布版本
    fun findTop1ByAgentIdAndPublishStatusAndDeletedAtIsNullOrderByVersionNumberDesc(agentId: String, publishStatus: Int): AgentVersionEntity?
